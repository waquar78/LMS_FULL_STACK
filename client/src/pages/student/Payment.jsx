...
import { useParams, useNavigate } from 'react-router-dom';   // <-- yaha se navigate hook
...
const Payment = () => {
  ...
  const paymentHandler = async (e) => {
    ...
    try {
      ...
      navigate("/");   // payment ke baad home page pe navigate
    } catch (error) {
      console.error(error);
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 w-96">
          ...
          <form onSubmit={paymentHandler}>
            <div className="mb-4">
              ...
              <input type="text" id="category" defaultValue={data.course.category} className="w-full px-4 py-2 border text-black rounded-lg" />
            </div>

            <div className="mb-4">
              <label htmlFor="product" className="block text-gray-700 font-medium mb-1">Course Id:</label>
              <input type="text" id="product" defaultValue={courseId} className="w-full px-4 py-2 border text-black rounded-lg" />
            </div>

            <div className="mb-4">
              <label htmlFor="amount" className="block text-gray-700 font-medium mb-1">Amount:</label>
              <input type="number" id="amount" defaultValue={data.course.coursePrice} className="w-full px-4 py-2 border text-black rounded-lg" />
            </div>

            <button type="submit" className="w-full bg-blue-600 text-white font-bold py-2 rounded-lg hover:bg-blue-700 transition">Pay</button>
          </form>
        </div>
      </div>
    </div>
